...
int findSide(int p1x,int p1y,int p2x,int p2y,int px,int py)
{
    ...
    if (val > 0) //outside triangle
        return 1;
    if (val < 0) //inside triangle
        return -1;
    return 0;
}
...
int setfunction(int hullx[],int hully[],int index,int convx[],int convy[]){
    ...
    for(int i=0;i<index-1;i++){ // max area  triangle
...
// End points of line L are p1 and p2.  side can have value 1 or -1 specifying each of the parts made by the line L
...
void findHull(int x[],int y[],int n,int *convx,int *convy,int *n_c)
{
    ...
    // Recursively find convex hull points on each side of line joining x[min_x] and x[max_x]
    ...
    *n_c=ind; // no. of points in the c_hull
    //printf("%d\n",ind);
